...
static volatile unsigned long ticksUp[3], ticksDown[3];                // in TMR1 ticks
...
JSN_SensLib::JSN_SensLib(uint8_t SYSTEM_USING) {
  ...
  switch(SYSTEM_USING) {
    ...
    case INPUT_CAPTURE:      
      ...
      TCCR1B |= (1 << CS01);            // Use 1:8 prescalar (TMR1 ticks @[f=2MHz],[T=0.5us])
...
ISR(TIMER1_CAPT_vect) {
  switch(READ_PIN4()) {
    ...
    case 0:
      ticksDown[0] = ICR1;
      echoHighTime[0] = ((ticksDown[0] - ticksUp[0]) >> 1);  // [2ticks = 1us]
